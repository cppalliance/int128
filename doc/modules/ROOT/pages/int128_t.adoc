...
Mixed-sign comparisons and arithmetic between `int128_t` and built-in unsigned types follow the C++ usual arithmetic conversions, identical to the built-in `__int128` type.
For comparisons and arithmetic with built-in unsigned types of lesser rank (`uint8_t` through `uint64_t`), `int128_t` is the common type and the operation is signed.
For operations with `unsigned __int128` (same rank), the signed `int128_t` is converted to the unsigned counterpart and the operation is unsigned, matching the C++ usual arithmetic conversions.
