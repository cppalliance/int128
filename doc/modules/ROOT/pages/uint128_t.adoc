...
Mixed-sign comparisons and arithmetic between `uint128_t` and built-in signed types follow the C++ usual arithmetic conversions, identical to the built-in `unsigned __int128` type.
The signed operand is converted to `uint128_t` (sign-extended for negatives) and the operation is performed unsigned, with two's-complement wrap-around semantics.
